...
export function validateUiSkill() {
  const errors = [];
...
if (invokedDirectly) {
  const errors = validateUiSkill();
  if (errors.length) {
    console.error('\nArco VXE UI skill validation failed:\n');
    for (const error of errors) console.error(`- ${error}`);
    console.error('');
    process.exit(1);
  }
  console.log('\nArco VXE UI skill validation passed.\n');
}
