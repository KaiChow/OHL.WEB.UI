...
export const QUERY_SCENARIO_SPEC = definePesdpPageSpec({
  ...
  pesdp: {
    ...
    structured: {
      ...
      acceptance: ['No scenario combines a flat query wall with a drawer, query actions keep one stable owner, pagination slices the rendered rows, and an empty result keeps reset recovery in the data surface.'],
    },
    dense: {
      ...
      acceptance: ['At 1024, 1366, 1440, and 1920 widths, role order remains stable; search/reset/expand/filter labels remain visible without clipping in zh-CN and en-US, pagination uses mini density, field rows use one shared 8px rhythm, and every adjacent query-action box uses the same 8px gap.', 'In S3 and S4 drawers, a normal single-line field measures 60px internally and adjacent rows have an 8px net gap; Arco default form-item margin never adds a second spacing layer.', 'At wide desktop widths the expanded S2 rows share one track model and retain only a normal final-row tail; the result header and body consume the full table owner width without a blank right-side column region; at narrow widths only the VXE table owns horizontal overflow.', 'The management result uses the shared stable row identity and hover behavior while inheriting the striped main-list baseline.'],
    },
    premium: {
