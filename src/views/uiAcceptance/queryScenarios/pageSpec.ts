...
export const QUERY_SCENARIO_SPEC = definePesdpPageSpec({
  ...
  pesdp: {
    ...
    dense: {
      ...
      acceptance: ['At 1024, 1366, 1440, and 1920 widths, role order remains stable; search/reset/expand/filter labels remain visible without clipping in zh-CN and en-US, pagination uses mini density, and the field-to-action gap stays at one shared gutter when S2 expands.', 'In S3 and S4 drawers, a normal single-line field measures 60px internally and adjacent rows have an 8px net gap; Arco default form-item margin never adds a second spacing layer.', 'At wide desktop widths the result header and body consume the full table owner width without a blank right-side column region; at narrow widths only the VXE table owns horizontal overflow.', 'The management result uses the shared stable row identity and hover behavior while inheriting the striped main-list baseline.'],
    },
    premium: {
