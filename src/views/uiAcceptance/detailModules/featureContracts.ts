...
export const DETAIL_MODULE_FEATURE_CONTRACTS = defineFeatureContracts([
  ...
  { id: 'detail-workspace-save', actorRoles: ['ui.acceptance.editor'], visibleWhen: '页面为默认编辑工作态', enabledWhen: '存在未保存修改、表单可提交且未在保存中', request: '保存本地验收页当前详情草稿', successResult: '提交当前编辑值、刷新已保存快照、清零未保存项计数、保持各模块业务模式并显示对象级成功反馈', errorResult: '保留全部编辑值与未保存项计数并在固定页脚显示可恢复失败', refreshScope: '对象头、受影响模块和固定页脚' },
