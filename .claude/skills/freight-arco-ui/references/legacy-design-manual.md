...
| 维度 | 定位 |
|------|------|
| 信息密度 | **Dense（高密度）**：每屏多展示数据；通过稳定的信息节奏保留字段、按钮、分区的可识别边界 |
...
- 下拉危险操作 → 使用 `danger-opt` 并二次确认；不可用 `status="danger"` 作为菜单项样式
...
.toolbar-aside { display:flex; align-items:center; gap:8px; margin-left:auto; }
```
...
> **高密度定位**：相对展示型 SaaS **收紧**间距与行高，让每屏多展示数据；同时保留字段、按钮、分区之间的可扫描节奏。
> 设计感来自：**固定间距阶梯 + 卡片分隔 + 对齐 + 名/值关系清晰**，而非大面积留白。

### 6.1 高密度的信息节奏（必须理解）

| 概念 | 含义 | 失效表现 |
|------|------|---------|
| **高密度** | 主列表行高 36px、字段行 gap 8px、模块 gap 8px（相对 48–56px 行高 / 24px gap 的展示型后台）| 字段、按钮、分区失去可辨认边界 |
| **紧凑** | 去掉无信息装饰、减少无效留白 | 字段名和输入面、相邻按钮、上下分区在视觉上合成一条带 |
| **有设计感** | 间距**可预期、可复用**（Token 阶梯），层级靠边框/阴影/背景区分 | 靠随机压小 padding，导致每个模块节奏不同 |
...
- ✅ 列表五层卡片之间仍有 **8px** 呼吸缝
- 表单 item 与 grid row-gap 要共同形成字段节奏；如果只压缩 margin 而没有 row-gap，字段组会丢失可扫描边界
- ❌ 去掉卡片圆角/阴影只靠「挤」来省空间
...
| `--dense-gap-label` | **4px** | 字段名 → 输入面的最小分组间隔，用于维持名/值关系 |
...
```
字段节奏失效：为「紧凑」只压缩 form-item margin，却没有用 grid gap 建立行间分组
...
| 筛选两行缺少分组节奏 | `filter-grid` row-gap 过小、body padding 不足 | row-gap **10px**、body **12×12px** |
...
<template v-if="selectedKeys.length > 0">
  ...
  <a-dropdown trigger="click" popup-class="row-action-menu">
    ...
    <template #content>
      ...
      <a-divider class="row-action-menu__divider" />
      <a-doption class="danger-opt" @click="bulkDelete">批量删除</a-doption>
    </template>
  </a-dropdown>
</template>
  ...
  <div class="row-actions">
    ...
    <a-dropdown trigger="click" popup-class="row-action-menu">
      ...
      <template #content>
        ...
        <a-divider class="row-action-menu__divider" />
        <a-doption class="danger-opt" @click="deleteRow(row)">删除</a-doption>
      </template>
    </a-dropdown>
  </div>
</template>
...
表格上方 toolbar：添加行 / 导入等（右对齐，作为表格操作带与表头邻近）
...
- ❌ 状态条单号 14px bold
- ❌ 模块汇总数值 15px / 700
- 字段行依赖 grid row-gap 建立扫描节奏；只压缩 form-item margin 会让字段组缺少层级
...
| 现象 | 根因 | 规范 |
|------|------|------|
| 选项组名/选项关系不清、选项密度失衡 | `gap: 6px 10px`、无 checkbox-group 内间距 | 容器 `gap: 8px 12px`；组内 **8×16px** |
...
| 现象 | 根因 | 规范 |
|------|------|------|
| 字段名与输入面横向混杂、列宽参差 | `a-form-item` **未包**在 `layout="vertical"` 的 `detail-form` 内 | 必须 `<a-form layout="vertical" class="detail-form">` + `detail-form-grid--N` |
...
> 子项/块内随表单或表格数据实时变化的统计，放在标题行或表头旁的同一信息带内。
...
<a-space :size="2">
  ...
  <a-dropdown trigger="click" popup-class="row-action-menu">
    ...
    <template #content>
      ...
      <a-doption class="danger-opt" @click="handleDelete(row)">删除</a-doption>
    </template>
  </a-dropdown>
</a-space>
```
...
- 分页位于**表格卡片顶部右侧**（`table-card-cap`），与表头保持同一卡片上下文，形成数据区控制带
...
| 间距 | `--dense-gap-zone` 8 / `--dense-gap-module` 8 / `--dense-gap-field-row` 8 / `--dense-gap-field-col` 12 / `--dense-gap-label` 4 | §6.2，用 token 维持信息节奏 |
...
- [ ] **间距 Token**：模块 gap 8px、表单 row 8/col 12、label 4px；字段、按钮、分区的节奏清晰（§6）
  ...
  <div class="toolbar-group">
    ...
    <template v-if="selectedRows.length > 0">
      ...
      <a-dropdown trigger="click" popup-class="row-action-menu">
        ...
        <template #content>
          <a-doption class="danger-opt" @click="bulkReject">批量拒绝</a-doption>
        </template>
      </a-dropdown>
