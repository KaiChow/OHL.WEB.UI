...
Use before implementing business clicks, requests, permissions, mutations, or state transitions. Every routed feature module declares typed feature contracts (for example a colocated `featureContracts.ts`) through the project's shared `defineFeatureContracts` helper; the contract is executable UI/UX infrastructure, not a prose design document.
...
Contract IDs are project-wide unique. Every routed `pageSpec.ts` action references one discovered contract ID, and the routed page imports its module's feature contracts instead of recreating visibility, enablement, pending, or transition rules locally.
...
The required fields, risk additions, and interaction rules in this file are the contract; the helper name and file paths are replaceable. A project adopting this skill must provide equivalent typed feature-contract infrastructure — a `defineFeatureContracts`-style helper with full type checking, a colocated contract-module convention per routed feature, and a static checker that verifies every `pageSpec.ts` action references a discovered contract ID.
