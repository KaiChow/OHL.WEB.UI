...
- 基础信息维护：客户分组、合同类型、字典、费用项、区域、标签…
- 用户/组织维护（无复杂权限矩阵时）
- 系统设置：参数分组、开关、集成项（低密度配置）

User job is **configure**, not high-frequency scan. Density may be slightly lower than the transaction list workbench, but still use project tokens — no `size="medium"`.
